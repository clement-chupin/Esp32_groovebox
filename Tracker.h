...
public:
  ...
  int patternLength;
  int trackIndex;
  CrunchVoice voices[4];
  Tracker();
  int UpdateTracker();
  void SetCommand(char command, int val);
  void ApplyPotControls(int masterRaw, int octaveRaw, int reverbRaw, int delayRaw, int phaserRaw);
  int scaleIndex;  // 0=chromatic 1=pentatonic 2=major
  int lastSamples[4];
...
private:
  ...
  int8_t trackFxPreset[4][512];
  int patternCopy[4][128];
  int patternCopyOctaves[4][128];
  int patternCopyInstruments[4][128];
  int8_t patternCopyFxPreset[4][128];
  int masterGainQ8;
  void ApplyFxPresetToTrack(int track, int preset);
  ...
  void SetTrackMute(int track, bool muteOn);
  void ChangeTrackGain(int track, int deltaQ8);
